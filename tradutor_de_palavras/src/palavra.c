...
void printWordsForSameSequence(NoTrie* raiz, const char* palavra) {
    ...
    for (int i = 0; sequence[i] != '\0'; i++) {
        ...
        if (!current->filhos[index]) {
            printf("\nPalavra informada: %s (seq: %s)\n", palavra, sequence);
            printf("Nenhuma palavra cadastrada para essa sequência.\n");
            free(sequence);
            return;
        }
    ...
    printf("\nPalavra informada: %s (seq: %s)\n", palavra, sequence);
    printf("Palavras já cadastradas para essa sequência:\n");
    bool found = false;
    ...
    if (!found) {
        printf("(nenhuma palavra cadastrada ainda)\n");
    }
    free(sequence);
}
