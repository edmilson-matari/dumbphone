...
void translateSequence(NoTrie* raiz, const char* sequence) {
    ...
    bool endsWithZero = (len > 0 && sequence[len - 1] == '0');
    int seqLen = endsWithZero ? len - 1 : len;

    // Navegar até o nó correspondente à sequência (considerando ou não o 0 final)
    ...
    if (endsWithZero) {
        // Exibir todas as palavras e alternativas para essa sequência
        bool found = false;
        NoTrie* alt = current;
        while (alt != NULL) {
        ...
        // Exibir apenas a primeira palavra
...
int main() {
    ...
    printf("=== SISTEMA TRADUTOR DE PALAVRAS ===\n");
            ...
            case 2:
                ...
                if (searchWord(system.raiz, palavra)) {
                    ...
                    if (sequence) {
                        ...
                        char* palavraLista = strtok(palavra, " ");
		        while (palavraLista != NULL) {
		            printWordsForSameSequence(system.raiz, palavraLista);
		            palavraLista = strtok(NULL, " ");
		        }
                        free(sequence);
                    } else {
